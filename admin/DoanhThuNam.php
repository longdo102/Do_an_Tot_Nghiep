...
<body class="theme-red">
    ...
    <section class="content">
        <div class="container-fluid">
           ...
           // Hàm lấy doanh thu theo năm trong 5 năm qua
            function getRevenueLast5Years($con) {
                $data = [];
                for ($i = 4; $i >= 0; $i--) {
                    ...
                    $sql = "SELECT SUM((od.quantity * p.price_new) - (od.quantity * p.price_nhap)) as revenue
                            ...
                            WHERE YEAR(o.created_time) = '$year'
                            AND o.status IN (1, 2)";
                    $result = $con->query($sql);
                    $row = $result->fetch_assoc();
                    $data[$year] = $row['revenue'] ? $row['revenue'] : 0;
                }
                return $data;
            }

            $yearly_revenue_data = getRevenueLast5Years($con);
            ?>

            <div class="row clearfix">
                <div class="row">
                    <div class="font-bold m-b-33 text-center">DOANH THU 5 NĂM QUA</div>
                </div>
    ...
    document.querySelector('#downloadPDF').addEventListener("click", function() {
        ...
        pdf.save("download.pdf");
                ...
                <script>
                    var revenueData = <?php echo json_encode($yearly_revenue_data); ?>;
            ...
            function countOrdersByStatus($con, $status) {
            ...
            $processing_orders = countOrdersByStatus($con, 0); 
            $shipping_orders = countOrdersByStatus($con, 1);    
            $completed_orders = countOrdersByStatus($con, 2); 
            $canceled_orders = countOrdersByStatus($con, 3);   
            ?>
