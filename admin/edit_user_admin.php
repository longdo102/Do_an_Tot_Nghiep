...
<html>
    <head>
        <title>Đổi thông tin admin</title>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            if (isset($_POST['id']) && !empty($_POST['id']) && isset($_POST['password']) && !empty($_POST['password'])) {
                $result = mysqli_query($con, "UPDATE `admin` SET `password` = ('" . $_POST['password'] . "'), `status` = " . $_POST['status'] . "WHERE `admin`.`id` = " . $_POST['id'] . ";");
                if (!$result) {
            ...
            <?php } else { ?>
                <div id="edit-notify" class="box-content">
                    ...
                    <a href="./nhanvien.php?id=<?= $_POST['id'] ?>">Quay lại sửa tài khoản</a>
                </div>
            <?php
            }
        } else {
            ...
            if (!empty($user)) {
                ...
                <div id="edit_user" class="box-content">
                    ...
                    <form action="./edit_user_admin.php?action=edit" method="Post" autocomplete="off">
